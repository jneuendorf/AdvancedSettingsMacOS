...
class InputError extends Error {
    constructor(message='Could not parse input.', ...args) {
        super(message, ...args)
    }
}
...
const stringifyBooleanInput = input => {
...
export default {
    System: {
        standbydelay: {
            ...
            command: `pmset -a standbydelay ${VALUE_PLACEHOLDER}`,
            type: 'number',
            stringifyInput: input => {
                const hours = parseFloat(input)
                if (isNaN(hours)) {
                    throw new InputError('Could not parse input for "hours".')
                }
                return hours * 3600
            },
            sudo: true,
    ...
    'General UI/UX': {
        reduceTransparency: {
            ...
            command: `defaults write com.apple.universalaccess reduceTransparency -bool ${VALUE_PLACEHOLDER}`,
            type: 'boolean',
            stringifyInput: stringifyBooleanInput,
        },
        AppleHighlightColor: {
            label: 'Set highlight color',
            command: `defaults write NSGlobalDomain AppleHighlightColor -string "${VALUE_PLACEHOLDER}"`,
            type: 'text',
            stringifyInput: identity,
